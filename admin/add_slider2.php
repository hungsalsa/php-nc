...
// echo '<pre>';print_r($_POST);
	...
	//         $uploadOk = 1;
	//     } else {
	//         $errors_anh =   "File is not an image.";
	//         $uploadOk = 0;
	//     }
	// }else {
	// 	$errors_anh =  "Bạn chưa chọn file ảnh.";
	// }

	// Check if file already exists
	if (file_exists($target_file)) {
	    $errors_anh =  "Sorry, file already exists.";
	    // echo "Sorry, file already exists.";
	    $uploadOk = 0;
	}
	// Check file size
	if ($_FILES["anh"]["size"] > 1000000) {
	    $errors_anh =  "Sorry, your file is too large. (<1M)";
	...
	&& $imageFileType != "gif" ) {
	    // $errors['anh'] =  "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
	    $errors_anh =  "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
	    $uploadOk = 0;
	}

	if ($uploadOk == 0) {
   		$errors['anh']=  $errors_anh;
	// if everything is ok, try to upload file
	} else {
	    if (move_uploaded_file($_FILES["anh"]["tmp_name"], $target_file)) {
	    	$image =  'uploads/'.$_FILES["anh"]["name"];
	        $_SESSION['anh'] = "The file ". basename( $_FILES["anh"]["name"]). " has been uploaded.";
	    } else {
	        $_SESSION['anh'] = "Sorry, there was an error uploading your file.";
	    }
	}
